...
class _NoopMqttClient implements MqttClientInterface {
  @override
...
class _PayloadCaptureClient extends TwoFinanceBlockchain {
  _PayloadCaptureClient()
    : super(
        keyManager: KeyManager(),
        mqttClient: _NoopMqttClient(),
        chainID: 1,
      );

  String? lastRequestMethod;
  dynamic lastRequestParams;
  ...
  Future<Uint8List> sendTransaction(
    ...
    lastRequestParams = tx;
    return Uint8List.fromList(utf8.encode('{}'));
  }
}

Future<_PayloadCaptureClient> _newPayloadCaptureClient() async {
  final c = _PayloadCaptureClient();
  final keyPair = await KeyManager().generateKeyEd25519();
  await c.setPrivateKey(keyPair.privateKey);
  return c;
}

Future<String> _newValidPublicKey() async {
  final keyPair = await KeyManager().generateKeyEd25519();
...
void main() {
  ...
  group('DropV1 FT Payloads', () {
    test('DepositDrop', () async {
      final c = await _newPayloadCaptureClient();
      final dropAddress = await _newValidPublicKey();
      final programAddress = await _newValidPublicKey();
      final tokenAddress = await _newValidPublicKey();
      ...
      expect(c.lastRequestMethod, equals(REQUEST_METHOD_SEND_TRANSACTION));

      final tx = c.lastRequestParams as Transaction;
      expect(tx.method, equals(METHOD_DEPOSIT_DROP));
      expect(tx.to, equals(dropAddress));
      expect(
        Map<String, dynamic>.from(tx.data),
        equals({
    ...
    test('WithdrawDrop', () async {
      final c = await _newPayloadCaptureClient();
      final dropAddress = await _newValidPublicKey();
      final programAddress = await _newValidPublicKey();
      final tokenAddress = await _newValidPublicKey();

      await c.withdrawDrop(
        address: dropAddress,
        programAddress: programAddress,
        tokenAddress: tokenAddress,
        amount: '200',
      ...
      final tx = c.lastRequestParams as Transaction;
      expect(tx.method, equals(METHOD_WITHDRAW_DROP));
      expect(tx.to, equals(dropAddress));
      expect(
        Map<String, dynamic>.from(tx.data),
        equals({
    ...
    test('DepositDrop with UUID', () async {
      final c = await _newPayloadCaptureClient();
      final dropAddress = await _newValidPublicKey();
      final programAddress = await _newValidPublicKey();
      final tokenAddress = await _newValidPublicKey();
      ...
      final tx = c.lastRequestParams as Transaction;
      final data = Map<String, dynamic>.from(tx.data);

      expect(data['uuid'], equals(const ['uuid-1']));
      expect(data.containsKey('uuids'), isFalse);
      expect(data.containsKey('token_uuid_list'), isFalse);
    });

    test('WithdrawDrop with UUID', () async {
      final c = await _newPayloadCaptureClient();
      final dropAddress = await _newValidPublicKey();
      final programAddress = await _newValidPublicKey();
      final tokenAddress = await _newValidPublicKey();
    ...
    test('ClaimDrop', () async {
      final c = await _newPayloadCaptureClient();
      final dropAddress = await _newValidPublicKey();

      await c.claimDrop(address: dropAddress);

      expect(c.lastRequestMethod, equals(REQUEST_METHOD_SEND_TRANSACTION));

      final tx = c.lastRequestParams as Transaction;
      expect(tx.method, equals(METHOD_CLAIM_DROP));
      expect(tx.to, equals(dropAddress));
      expect(Map<String, dynamic>.from(tx.data), isEmpty);
    });
    ...
    test('GetDrop', () async {
      ...
      await c.getDrop(address: dropAddress);

      expect(c.lastRequestMethod, equals(REQUEST_METHOD_GET_STATE));

      final request = Map<String, dynamic>.from(c.lastRequestParams as Map);
      expect(request['to'], equals(dropAddress));
      expect(request['method'], equals(METHOD_GET_DROP));
      expect(Map<String, dynamic>.from(request['data'] as Map), isEmpty);
    });
  });
}