...
void main() {
  group('DecimalRescaler.rescaleString', () {
    ...
    test('downscale underflow returns 0 when combined length <= -diff', () {
      ...
      expect(DecimalRescaler.rescaleString('0.01', 2, 0), '0');

      // "0.0000001" from=7 => "1"; to=0 => diff=-7 => underflow => "0"
      expect(DecimalRescaler.rescaleString('0.0000001', 7, 0), '0');
    });

    test('invalid numeric input throws FormatException', () {
      expect(
        () => DecimalRescaler.rescaleString('12a.34', 2, 2),
        throwsA(isA<FormatException>()),
      );

      expect(
        () => DecimalRescaler.rescaleString('1.2.3', 2, 2), // split => "1" + "2" but extra dot breaks numeric
        throwsA(isA<FormatException>()),
      );
    });
